...
class CategorieEventController extends AbstractController
{
    ...
    /**
     * @Route("/{idCategEvent}", name="categorie_event_show", methods={"GET"})
     */
    ...
    /**
     * @Route("/{idCategEvent}/edit", name="categorie_event_edit", methods={"GET","POST"})
     */
    ...
    /**
     * @Route("/{idCategEvent}", name="categorie_event_delete", methods={"POST"})
     */
    public function delete(Request $request, CategorieEvent $categorieEvent): Response
    {
        if ($this->isCsrfTokenValid('delete'.$categorieEvent->getIdCategEvent(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
